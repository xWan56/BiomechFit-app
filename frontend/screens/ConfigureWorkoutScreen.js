...
export default function ConfigureWorkoutScreen({ route, navigation }) {
  ...
  const [recommendedSets, setRecommendedSets] = useState(null);
 ...
 const handleAgeChange = (value) => {
  ...
  if (value === "") {
    setAge("");
    setAgeError("");
  } else if (!isNaN(ageNumber) && ageNumber <= 60) {
    setAge(numericValue);
    setAgeError("");
  } else if (ageNumber > 60) {
    setAge(numericValue);
    setAgeError("Age must not exceed 60 years old.");
  }
};
const handleRepsChange = (value) => {
  ...
  if (value === "") {
    setReps("");
    setRepsError("");
  } else if (!isNaN(repsNumber) && repsNumber <= 13) {
    setReps(numericValue);
    setRepsError("");
  } else if (repsNumber > 13) {
    setReps(numericValue);
    setRepsError("Reps must not exceed 13.");
  }
};
  ...
  const handleStartExecution = async () => {
    ...
    try {
      ...
      setRecommendedReps(result.recommendation.recommended_reps);
      setRecommendedSets(result.recommendation.recommended_sets);
      setRecommendedWeight(result.recommendation.recommended_weight);  
...
backButton: {
  position: "absolute",
  top: 20,               // Adjust depending on your header height
  left: 20,
